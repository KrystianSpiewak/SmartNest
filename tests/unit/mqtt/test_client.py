...
@pytest.fixture
def client(mqtt_config: MQTTConfig, mock_paho_client: MagicMock) -> SmartNestMQTTClient:
    """SmartNestMQTTClient with a mocked Paho client."""
    with patch("backend.mqtt.client.mqtt.Client", return_value=mock_paho_client):
        return SmartNestMQTTClient(mqtt_config)
...
class TestClientInit:
    ...
    def test_creates_paho_client_with_v2_api(self, mqtt_config: MQTTConfig) -> None:
        with patch("backend.mqtt.client.mqtt.Client") as mock_cls:
            ...
            SmartNestMQTTClient(mqtt_config)
            mock_cls.assert_called_once_with(
                callback_api_version=CallbackAPIVersion.VERSION2,
                client_id="test_client",
            )

    def test_enables_paho_logger(
    ...
    def test_sets_lwt(self, client: SmartNestMQTTClient, mock_paho_client: MagicMock) -> None:
        ...
        call_args = mock_paho_client.will_set.call_args
...
class TestClientProperties:
    ...
    def test_config_returns_config(
        self, client: SmartNestMQTTClient, mqtt_config: MQTTConfig
    ) -> None:
        assert client.config is mqtt_config
...
class TestClientConnect:
    ...
    def test_connect_calls_paho_connect(
        ...
        mock_paho_client.loop_start.side_effect = set_connected
        ...
        mock_paho_client.connect.assert_called_once_with("localhost", 1883, keepalive=60)
...
class TestClientPublish:
    ...
    def test_publish_with_custom_qos_and_retain(
        ...
        mock_paho_client.publish.assert_called_once_with(
            "t", json.dumps({"x": 1}), qos=0, retain=True
        )
    ...
    def test_publish_failure_returns_false(
        ...
        client.set_connected_for_test()
        mock_paho_client.publish.return_value = MagicMock(rc=mqtt.MQTT_ERR_NO_CONN)
...
class TestClientSubscribe:
    ...
    def test_subscribe_when_connected(
        ...
        with patch("backend.mqtt.client.log_with_code") as mock_log:
            ...
            assert result is True
            mock_paho_client.subscribe.assert_called_once_with("smartnest/device/+/state", qos=1)
    ...
    def test_subscribe_failure_returns_false(
        ...
        client.set_connected_for_test()
        mock_paho_client.subscribe.return_value = (mqtt.MQTT_ERR_NO_CONN, None)
    ...
    def test_subscribe_default_qos(
        self, client: SmartNestMQTTClient, mock_paho_client: MagicMock
    ) -> None:
...
class TestClientTopicHandlers:
    ...
    def test_remove_topic_handler(
        self, client: SmartNestMQTTClient, mock_paho_client: MagicMock
    ) -> None:
        client.remove_topic_handler("smartnest/device/+/state")
        mock_paho_client.message_callback_remove.assert_called_once_with("smartnest/device/+/state")
...
class TestClientCallbacks:
    ...
    def test_on_connect_success_sets_connected(
        self, mock_paho_client: MagicMock, mqtt_config: MQTTConfig
    ) -> None:
        """Connection success must log broker, port, and client_id for observability."""
        with patch("backend.mqtt.client.mqtt.Client", return_value=mock_paho_client):
            snc = SmartNestMQTTClient(mqtt_config)

        with patch("backend.mqtt.client.log_with_code") as mock_log:
            on_connect = mock_paho_client.on_connect
            reason = ReasonCode(mqtt.CONNACK >> 4, identifier=0)
            on_connect(mock_paho_client, None, MagicMock(spec=ConnectFlags), reason, None)
    ...
    def test_on_disconnect_clears_connected(
        ...
        on_disconnect(mock_paho_client, None, MagicMock(spec=DisconnectFlags), reason, None)
        assert snc.is_connected is False
    ...
    def test_on_message_logs_unhandled(
        self, mock_paho_client: MagicMock, mqtt_config: MQTTConfig
    ) -> None:
        ...
        with patch("backend.mqtt.client.log_with_code") as mock_log:
            ...
            on_message(mock_paho_client, None, msg)
...
class TestMessagePayloadDecoding:
    ...
    def test_payload_truncated_at_exactly_200_chars(
        ...
        with patch("backend.mqtt.client.log_with_code") as mock_log:
            client._on_message(mock_paho_client, None, msg)
    ...
    def test_payload_short_not_truncated(
        self, mock_paho_client: MagicMock, mqtt_config: MQTTConfig
    ) -> None:
